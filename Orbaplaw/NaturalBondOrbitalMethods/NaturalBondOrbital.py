...
def generateNaturalBondOrbital(basis_indices_by_frag,P,C,S,maxnfrags,maxnnbos,threshold):
    ...
    I[:,:nnhos]=I[:,:nnhos]@Onho # Transforming the coefficient matrix of NHO to that of orthogonal NHO
    ...
    for nho_indices_comb in nho_indices: # NHO indices of this combination, [fragments] -> [[NHO]]
        ...
        for i in range(len(Nblock)+1):
            if ( (not np.isclose(degenerate[0],Nblock[i])) if i<len(Nblock) else i==len(Nblock) ) and degenerate[1]!=[]:
...
def NaturalBondOrbital(nao_mwfn,frags=[],maxnfrags=-1,maxnnbos=-1,threshold=0.0075): # By default, every atom is a fragment, which is the case of NBO. By combining atoms into fragments one extends NBO to natural fragment bond orbital (NFBO).
    ...
    if nao_mwfn.Wfntype==0:
        ...
        NHO_nao,NBO_n,NBO_nao=generateNaturalBondOrbital(basis_indices_by_frag,P,C,S,maxnfrags,maxnnbos,threshold)
