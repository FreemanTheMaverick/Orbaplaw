...
class MultiWaveFunction:
    ...
    def getBasisIndexByCenter(self,center):
        assert type(center) is MwfnCenter or int
        thiscenter=None
        if type(center) is MwfnCenter:
            assert center in self.Centers
            thiscenter=center
        else:
            thiscenter=self.Centers[center]
        ibasis=0
        indices=[]
        for shell in self.Shells:
